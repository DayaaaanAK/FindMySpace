...
const signup = async (req, res) => {
  try {
    ...
    return res.status(201).json({ message: "Landlord signup successful." });
  } catch (error) {
    console.error(error);
...
const updateProfile = async (req, res) => {
  try {
    ...
    const existingProfile = await LandlordProfile.findOne({ user: userId });
    ...
    const updatedProfile = await LandlordProfile.findByIdAndUpdate(
      existingProfile._id,
      { $set: updateFields },
